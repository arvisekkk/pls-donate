...
print('leak by: asterek#0002  |  https://sheecord.pl/discord/')
...
queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/arvisekkk/pls-donate/main/easyrobux.lua'))()")
...
local function webhook(raised,donor)
	...
	local a = {
		...
		["footer"] = {
			["text"] = "leak by: asterek#0002  |  https://sheecord.pl/discord/",
		},
  ...
  --GUI
local Window = library:AddWindow("leak by: asterek#0002  |  https://sheecord.pl/discord/",
  {
...
webhookTab:AddButton("Test Webhook", function()
	if getgenv().settings.webhookBox then
		oldWebhook("Webhook works | LEAK BY: asterek#0002 (https://sheecord.pl/discord/)")
	end
end)
...
if game:GetService("CoreGui").imgui.Windows.Window.Title.Text == "Loading..." then
	game:GetService("CoreGui").imgui.Windows.Window.Title.Text = "Leaked by asterek#0002   |   PLS DONATE"
end
